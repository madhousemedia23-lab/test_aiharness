...
Create `.ai/config/project.json` with this exact structure (replace placeholder values):
...
Run the following, replacing `<repo_url>` with the saved URL:
...
- Git remote `origin` set to `<repo_url>`
- **Password / PAT will be asked every time before a push — it is never stored anywhere**
...
- When pushing, ask for the PAT at push time and use it once via the remote URL:
