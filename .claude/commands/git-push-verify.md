...
**Push transport:** `github.push_transport` is `"https"` (default) or `"ssh"`. HTTPS uses a one-shot PAT in the push URL. SSH uses your local SSH keys; the first connection may show a host-key fingerprint — compare to [GitHub’s published SSH key fingerprints](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/githubs-ssh-key-fingerprints) before typing `yes`.

---

## Step 1 — Confirm repo config

Read `.ai/config/project.json`. Extract `github.repo_url`, `github.username`, and `github.push_transport` (treat missing as `"https"`).
...
Run this commit command exactly. It is the canonical record of everything built in this project, with no AI references in the body.
...
- /github-setup command: interactive repo connection; HTTPS+PAT or SSH push (see `push_transport`)
...
Resolve the current branch: `BRANCH=$(git branch --show-current)` (use this instead of assuming `master`).

### Step 5a — HTTPS (`push_transport` missing or `"https"`)

Tell the user:
> "Ready to push to `<repo_url>` over HTTPS. Please paste your GitHub Personal Access Token (it will be used once and never stored anywhere):"
...
git push "https://<username>:<PAT>@github.com/${REPO_PATH}.git" --set-upstream origin "$BRANCH" 2>&1 | sed 's/<PAT>/****/g'
```
...
### Step 5b — SSH (`push_transport` is `"ssh"`)

1. Confirm `git remote get-url origin` is an SSH URL (`git@github.com:<owner>/<repo>.git`). If it is still HTTPS, set it:
   ```bash
   REPO_PATH=$(echo "<repo_url>" | sed 's|https://github.com/||')
   git remote set-url origin "git@github.com:${REPO_PATH}.git"
   ```
2. Tell the user: push will use the SSH agent / default key; first connection may ask to verify GitHub’s host key — they should compare `SHA256:…` to GitHub’s docs and type `yes` if it matches.
3. Push (no PAT):
   ```bash
...
git log "origin/${BRANCH}" --oneline -8
```
...
# 2. Commit graph on origin (current branch)
BRANCH=$(git branch --show-current)
git log "origin/${BRANCH}" --oneline --graph -10

# 3. Repo stats via GitHub CLI (if installed)
gh repo view 2>/dev/null

# 4. Fallback if gh not available
gh repo view 2>/dev/null || python3 -c "import json; print(json.load(open('.ai/config/project.json'))['github']['repo_url'])"
```
...
- Change to `- [x]` and set the **actor marker on that task line** to match the harness that ran this workflow: **`{cursor}`** if run from Cursor, **`{claude}`** if run from Claude Code (see `.ai/protocols/TODO_MD_AGENT_PROTOCOL.md`)
- Add `# ai:` note: `Committed feat: project harness v1.0, pushed to <repo_url>, verified on origin/<branch>.`
